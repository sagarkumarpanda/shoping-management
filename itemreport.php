...
{
	$item=$_POST['text1'];
	$y="select * from items where pname='$item'";
	$yy=mysqli_query($con,$y);
for($i=1;$i<100;$i++){
while($row=mysqli_fetch_row($yy))
{
echo("<form action='http://localhost:8080/project/itemaction.php' method='post' >");
echo("<tr>");
echo("<td>".$i."</td>");
echo("<td>".$row[1]."</td>");
echo("<td>".$row[2]."</td>");
...
echo("<td><input type='submit' value='Sales' name='sales' class='sales'>");
echo("</tr>");
echo("</form>");
$i++;
}
